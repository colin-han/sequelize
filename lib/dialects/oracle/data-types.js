...
module.exports = BaseTypes => {
  ...
  BaseTypes.DATE.types.oracle = [43];
  BaseTypes.STRING.types.oracle = [231, 173];
  BaseTypes.CHAR.types.oracle = [175];
  BaseTypes.TEXT.types.oracle = false;
  BaseTypes.INTEGER.types.oracle = [38];
  BaseTypes.BIGINT.types.oracle = false;
  BaseTypes.FLOAT.types.oracle = [109];
  BaseTypes.TIME.types.oracle = [41];
  BaseTypes.DATEONLY.types.oracle = [40];
  BaseTypes.BOOLEAN.types.oracle = [104];
  BaseTypes.BLOB.types.oracle = [165];
  BaseTypes.DECIMAL.types.oracle = [106];
  BaseTypes.UUID.types.oracle = false;
  BaseTypes.ENUM.types.oracle = false;
  BaseTypes.REAL.types.oracle = [109];
  BaseTypes.DOUBLE.types.oracle = [109];
  ...
  BLOB.prototype.toSql = function toSql() {
    if (this._length) {
      if (this._length.toLowerCase() === 'tiny') { // tiny = 2^8
        warn('ORACAL does not support BLOB with the `length` = `tiny` option. `VARBINARY(256)` will be used instead.');
        return 'VARBINARY(256)';
      }
      warn('ORACAL does not support BLOB with the `length` option. `VARBINARY(MAX)` will be used instead.');
    }
  ...
  TEXT.prototype.toSql = function toSql() {
    ...
    if (this._length) {
      if (this._length.toLowerCase() === 'tiny') { // tiny = 2^8
        warn('ORACAL does not support TEXT with the `length` = `tiny` option. `NVARCHAR(256)` will be used instead.');
        return 'NVARCHAR(256)';
      }
      warn('ORACAL does not support TEXT with the `length` option. `NVARCHAR(MAX)` will be used instead.');
    }
    return 'NVARCHAR(MAX)';
  };
  ...
  function INTEGER(length) {
    ...
    // ORACAL does not support any options for integer
    if (this._length || this.options.length || this._unsigned || this._zerofill) {
      warn('ORACAL does not support INTEGER with options. Plain `INTEGER` will be used instead.');
  ...
  function BIGINT(length) {
    ...
    // ORACAL does not support any options for bigint
    if (this._length || this.options.length || this._unsigned || this._zerofill) {
      warn('ORACAL does not support BIGINT with options. Plain `BIGINT` will be used instead.');
      this._length = undefined;
      this.options.length = undefined;
      this._unsigned = undefined;
      this._zerofill = undefined;
  ...
  function REAL(length, decimals) {
    ...
    // ORACAL does not support any options for real
    if (this._length || this.options.length || this._unsigned || this._zerofill) {
      warn('ORACAL does not support REAL with options. Plain `REAL` will be used instead.');
      this._length = undefined;
      this.options.length = undefined;
      this._unsigned = undefined;
      this._zerofill = undefined;
  ...
  function FLOAT(length, decimals) {
    ...
    // ORACAL does only support lengths as option.
    // Values between 1-24 result in 7 digits precision (4 bytes storage size)
    // Values between 25-53 result in 15 digits precision (8 bytes storage size)
    // If decimals are provided remove these and print a warning
    if (this._decimals) {
      warn('ORACAL does not support Float with decimals. Plain `FLOAT` will be used instead.');
      this._length = undefined;
      this.options.length = undefined;
    }
    if (this._unsigned) {
      warn('ORACAL does not support Float unsigned. `UNSIGNED` was removed.');
      this._unsigned = undefined;
    }
    if (this._zerofill) {
      warn('ORACAL does not support Float zerofill. `ZEROFILL` was removed.');
      this._zerofill = undefined;
    }
  }
