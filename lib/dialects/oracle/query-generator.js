...
const QueryGenerator = {
  ...
  selectQuery(tableName, options, model) {
    ...
    if (options.group) {
      options.group = Array.isArray(options.group) ? options.group.map(function(t) { return this.quote(t, model); }.bind(this)).join(', ') : this.quote(options.group);
      if (subQuery) {
  ...
  bulkInsertQuery(tableName, attrValueHashes) {
    let query = 'INSERT ALL <%= tuples %> SELECT * FROM dual;';
    let attributesTpl = 'INTO <%= table %> (<%= columns %>)'
      , tuples = []
      , allAttributes = []
    ;
    ...
    let attributes = _.template(attributesTpl)({
      table:this.quoteIdentifier(tableName),
  ...
  attributeToSQL(attribute) {
    ...
    if (attribute.type instanceof DataTypes.ENUM) {
      ...
      if (Array.isArray(attribute.type.values) && (attribute.type.values.length > 0)) {
        template = 'VARCHAR2('+len+') CHECK( "'+attribute.name+'" IN (' + _.map(attribute.type.values, function(value) {
